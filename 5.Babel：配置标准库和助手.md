...
当你回过头看，所有通过`transform-runtime`生成的引用从`babel-runtime/core-js`开始使用模块ID。这是因为Babel的polyfilling是基于`core-js`库的。你能翻阅`transform-runtime`如果详细遍历各定义并在仓库文件`definitions.js`中链接各属性接入至`core-js`模块。这是一个摘录： 
